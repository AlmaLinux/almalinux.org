...
We are shipping patched kernels for **two** serious Linux kernel vulnerabilities and would like your help verifying them before they go to production. The first, **Januscape** ([CVE-2026-53359](https://www.cve.org/CVERecord?id=CVE-2026-53359)), is a KVM/x86 guest-to-host escape: an attacker who can start a virtual machine can break out and run commands as root on the host. The second, **Bad Epoll** ([CVE-2026-46242](https://github.com/J-jaeyoung/bad-epoll)), is a use-after-free in the kernel's epoll subsystem that lets an unprivileged local user escalate to root.
