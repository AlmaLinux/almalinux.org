...
The 404 page includes a closest-match redirect for mistyped or incomplete URLs.

- Hugo now emits a page `index.json` alongside the normal HTML output. That JSON is generated by `layouts/_default/index.json` and contains each regular page's title and relative permalink.
- `layouts/404.html` fetches that index in the browser, tries a simple prefix match first, then falls back to Fuse.js fuzzy matching on the requested path and page title.
- If a strong match is found, the 404 message is replaced with a short redirect notice and the visitor is sent to the best match automatically. If no match clears the threshold, the page stays on the normal 404 view.
- No manual index maintenance is needed when pages are added or renamed; rebuilding the site regenerates the JSON index automatically.
